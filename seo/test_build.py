...
class GuideChecks(unittest.TestCase):
    ...
    def test_preview_and_production_have_different_indexing(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn('<script defer src="/analytics.js?v=stats1"></script>', html)
            self.assertNotIn("googletagmanager", html)
            self.assertNotIn("gtag(", html)
            self.assertNotIn('id="analytics-choice"', html)
            self.assertNotIn('data-analytics-choice', html)
            self.assertIn('data-analytics-toggle', html)
            self.assertIn('Stop site analytics', html)
