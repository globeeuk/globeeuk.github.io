...
- Forty-one licensed photo assets now cover 46 current cards: 36 venue photos, nine location photos and one activity reference photo. Sixteen cards without a sufficiently specific, reusable source keep a labelled illustration. Repeated venue groups use different views where possible, bringing the directory to 47 distinct displayed image assets. No venue/location photograph is described as the current event.
...
- Twenty automated tests pass: existing DB/consent/loading regressions, weekend boundaries and deduplication, real-region selection counts, full batch traversal, image completeness/licences/size and source-register coverage, all-map selection, zoom grouping and location validation. The saved-data error notice and retry remain available; successful update dates are hidden.
