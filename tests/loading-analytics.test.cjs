...
test('All public entry pages use the revised settings control and script',()=>{for(const name of ['index.html','plans.html','halloween.html','privacy.html','exeter/index.html','exeter/this-weekend/index.html','exeter/free-things-to-do/index.html','exeter/rainy-day/index.html','bristol/index.html','bristol/this-weekend/index.html']){const html=source(name);assert(html.includes('data-analytics-toggle'),name);assert(html.includes('analytics.js?v=consent3'),name);assert(html.includes('basic-analytics.js?v=basic1'),name);assert(html.indexOf('basic-analytics.js')<html.indexOf('analytics.js?v=consent3'),name);assert(!html.includes('Stop site analytics</button>'),name);}});
