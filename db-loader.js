...
(function(){
  ...
  function status(message,retry=false){const el=document.getElementById('database-status');el.hidden=!retry;el.innerHTML='';el.append(document.createTextNode(message));if(retry){const b=document.createElement('button');b.className='text-button';b.textContent='Refresh';b.addEventListener('click',()=>refresh(true));el.append(b);}}
